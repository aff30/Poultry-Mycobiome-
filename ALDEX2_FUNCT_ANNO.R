##how many ASVs were annotated to total of KEGG categories?
library(tidyverse)
...
sig_kw_annot <- read_csv("ALDEx2_KEGG_results.csv", show_col_types = FALSE)

# Rank and select top 20
top20 <- sig_kw_annot %>%
  mutate(effect_size = abs(we.eBH)) %>%
  arrange(we.ep) %>%
  slice_head(n = 20) %>%
  mutate(
    label = coalesce(type, path_id, ko_id),
    label = str_trunc(label, 60, "right"),
    label = fct_reorder(label, -log10(we.ep))
  )

# ---- Plot ----
p <- ggplot(top20, aes(x = -log10(we.ep), y = label, fill = broadgroup)) +
  geom_col(color = "black", width = 0.7) +
  scale_fill_brewer(palette = "Set3", name = "KEGG Broadgroup") +
  labs(
    x = expression(-log[10]("p-value")),
    y = "KEGG Pathway",
    title = "Top 20 Differential KEGG Pathways (ALDEx2)",
    subtitle = "Based on Kruskal–Wallis test across treatments"
  ) +
