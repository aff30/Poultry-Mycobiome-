...
# setwd
setwd("~/OneDrive - The Pennsylvania State University/Shared-Ana-Sophia/Functional_Chicken/FuncAnnoTest/")

# ---- Load tables and combine ----
path <- "output_funcprofiler/"
...
met<- read.csv("met/turkey_metadata.csv")

combined2 <- combined %>%
  mutate(Project_ID = str_remove_all(file, "_temp.tsv")) %>%
  # add metadata
  inner_join(met, by = "Project_ID") %>%
  # add ko information
  inner_join(koterm %>%
               mutate(ko_id = V2,
                      path_id = V1) %>%
  ...
  select(Project_ID, Sample_Type, Treatment, ko_id, abundance, path_id, map_ID, broadgroup, subgroup, type, type1, flags)
